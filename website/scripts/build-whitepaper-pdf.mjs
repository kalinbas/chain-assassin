...
const inputMdPath = path.join(repoRoot, 'WHITEPAPER.md');
const outDir = path.join(repoRoot, 'docs', 'whitepaper');
const outHtmlPath = path.join(outDir, 'ChainAssassin-Whitepaper-v1.html');
const outPdfPath = path.join(outDir, 'ChainAssassin-Whitepaper-v1.pdf');
...
main().catch((err) => {
  console.error('Failed to build whitepaper PDF.');
  console.error(err);
  process.exit(1);
});
